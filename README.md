...
No accounts, cookies, analytics, localStorage, or remote gameplay telemetry. The privacy trace is deliberately dull: [`privacy.txt`](privacy.txt). The public state trace is just as dull: [`status.txt`](status.txt). The deployment trace is dull too: [`provenance.txt`](provenance.txt). Dull is good here. Dull means the page is not pretending to be free while quietly renting a basement.
