...
## Deployment provenance trace audit — 2026-06-21

Target: repair a concrete source/deploy trace weakness without reopening the pre-gate design spiral. The doorway named source, status, issue surface, privacy boundary, and amendment reason. It still did not expose the dull live question: which repository branch, workflow, and source revision produced the static object now served by Pages.

### Pre-run review
No Stefano playtest feedback, issue, PR, or coding hint appeared. Gate freeze still holds. Today's status/certification thread sharpened the useful cold row: author, time, reason, revocation, version, and the question of whether a root only judges itself. For The Procedure this does not justify another case, mechanic, or explanatory page. It exposes one material doorway defect: the live page pointed to source, but did not carry a deploy provenance handle tying the served object back to a specific source revision generated by the Pages workflow.

### Autoprompt
Treat the live static page as an exported record, not as a trusted oracle. If provenance changes the first surface, it is too warm. If it gives archives, cautious players, and future maintainers a plain handle to compare the served page with its source, add it. Let the workflow write the exact live revision; do not pretend a committed file can honestly contain its own future hash. Then stop. The machine does not need another sermon about accountability. It needs a receipt.

### Audit result
- **The visible first surface stays unchanged.** No banner, helper copy, onboarding text, or diagnostic wrapper was added.
- **The project now has a plain provenance trace.** `provenance.txt` names repository, branch, deployment workflow, correction surface, data boundary, and license.
- **The live trace will carry exact source revision.** The Pages workflow overwrites `provenance.txt` during deployment with `GITHUB_SHA`, generation time, and workflow run URL before uploading the static artifact.
- **The page now carries machine-readable provenance handles.** `index.html` has a `provenance-trace` meta field, JSON-LD `codeRepository`, and a `subjectOf` link to the provenance trace.
- **Status and sitemap now include the provenance surface.** `status.txt` points to the deploy trace; `sitemap.xml` includes it and moves public freshness to `2026-06-21`.
- **Local metadata/provenance validation passed.** JSON-LD parsed, provenance metadata was present, `provenance.txt` existed locally, workflow YAML parsed, and sitemap parsed as XML.
- **Local visual smoke test passed.** The standby first screen still shows the low-current terminal; the added deploy trace is inspectable but not visible.
...
- **Live deployment passed.** The Pages workflow generated the live provenance and artifact manifest from `dadd6c275df55d77c0e16a15ab474798c8c7f7ef`; live `artifact-manifest.txt`, `provenance.txt`, and `status.txt` returned 200.
- **First screens hold.** Local and live desktop/mobile screenshots still show the standby machine: low current, power available, awaiting operator. No wrapper, no pitch, no checksum clerk at the door.
