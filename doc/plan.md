...
- [x] 2026-07-07 deployment recovery review held: the db4acda Pages failure was transient, not a design or workflow defect. A later manual `Deploy static Pages` run succeeded, live `review-window.txt`, `status.txt`, and `provenance.txt` all return 200, and the live provenance names source revision `db4acda3aa357634d8af9a915ccf930a9d2ba4cb`. The live first surface still opens on the cold standby machine. No new pre-gate design work justified.
