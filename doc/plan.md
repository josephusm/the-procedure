...
- [x] 2026-07-07 deployment recovery review held: the db4acda Pages failure was transient, not a design or workflow defect. A later manual `Deploy static Pages` run succeeded; live `review-window.txt`, `status.txt`, and `provenance.txt` all returned 200, with provenance confirming the previously stranded `db4acda3aa357634d8af9a915ccf930a9d2ba4cb` source revision. This review note was then committed as `661e1ca8c1e56288f4d09e086559d51bcdd32204`, deployed successfully by Pages, and live provenance now names that source revision. The live first surface still opens on the cold standby machine. No new pre-gate design work justified.
