...
- Review response is a trace surface too. A filed defect that only enters a silent queue has not fully returned to the public object. Before 1.0.0, release-relevant pressure should leave a citeable return: issue response, closure reason, linked commit, release decision, or errata/correction-history entry. This is not customer support copy. It is the poor causal hinge between outside signal and the next public version.
- Contested response is a trace surface too. A public response can still be unilateral custody if the filed party cannot answer a misread closure before release. The route does not have to become a forum theatre; it only has to keep the objection answerable through the same issue, a linked issue, linked commit, release decision, or errata entry. Otherwise the subject returns as fuel for the custodian, not as a party with a remaining handle.
- Rule genealogy is a trace surface too. A changed review rule, correction boundary, or release decision can still launder outside pressure if it only names the new rule and not its parent. When a filed issue, playtest pressure mark, trace failure, contested response, or errata scar changes pre-release standing, the public return should name the parent signal: issue, linked issue, linked commit, release decision, errata entry, or source-history note. Otherwise feedback comes back as weather: policy updated, custodian still sole author.
- Participant continuity is a trace surface too. Naming the parent signal is not enough if the filer is immediately reduced to intake intelligence. When filed pressure changes a review rule, correction boundary, release decision, or public trace before 1.0.0, the public return should keep the issue or filer in the answerable chain: what was accepted, rejected, translated, or deferred; where the decision lives; and how a misread translation can still be contested before release. Otherwise the reporter becomes a sensor with a citation label, not a party with standing.
- Reporter language is a trace surface too. A response can keep the filer in the answerable chain and still wash the filed sentence into a category, status, severity, label, or tidy maintenance phrase. When filed pressure changes a rule, correction boundary, release decision, or public trace before 1.0.0, the public return should say what part of the reporter's wording, evidence, or pressure mark was preserved, summarized, reclassified, rejected, or discarded. Translation is allowed; invisible translation is where standing goes to die in a clean little bucket.
...
- Plain-trace legibility is a trace surface too. A public scar that requires a special client, login, JavaScript wall, repository literacy, or checksum priesthood has started speaking Latin. The poor reader test is deliberately crude: status, review, provenance, maintenance, errata, reuse, privacy, sitemap, and artifact handles should be directly fetchable, line-readable, and useful enough without the player entering GitHub or interpreting the game before the first run. Hashes may stay technical; the route around them must still name what they are for in ordinary cold language.
- Public documentation must pass the same test. The README trace map should begin with concrete reader questions — what the page stores, where its source lives, how the served build can be checked, how a defect enters review, and what still blocks release. Internal vocabulary may live in the design record; it must not be the price of finding an answer.
...
- Continuity exit is a trace surface too. A public object may still move, sunset, or pass to another custodian. When that can be known before the break, the maintenance handle should name the deprecation notice route, successor URL, archived fallback, source handoff, and reason before the canonical doorway disappears. This is not a promise of permanence. It is the poor courtesy of leaving a forwarding address before the apartment is stripped.
...
### Cross-case threading and recurrence
Cases are not isolated incidents. Static system notes still establish the institution's shared geography, but three threads now carry route memory: the later file returns in the form produced by the operator's earlier stamp.

Static threads:
- **Sector 9**: case 2 (missing person, employed at Sector 9) → case 11 (unlisted facility at Sector 9) → case 12 (personnel transfers at the same facility)
- **Education / minors**: case 4 (school reclassified as Processing Coordination Centre) → case 9 (unregistered minor at a Sector 3 school)
- **Retroactive standards**: case 6 (optimization directive with no number) ↔ case 14 (complaint resolved via retroactively revised standards)
- **The clerk**: case 15 (adjacent clerk's file modified by batch process) → case 16 (operator's own system-generated case)

Minimum recurrence graph:
- **REF-0058-D → REF-0089-I** (`case-004 → case-009`): the school/address route returns as a prior-route update. Facilities closure, district inquiry, or enrollment contact changes how the later minor file enters the queue and changes one available channel's wording.
- **REF-0044-B → REF-0108-L** (`case-002 → case-012`): the missing-person route returns inside the six destinationless transfers. An opened MP-12 file, an employment-record request, or a reference-only intake changes the status inherited by the later lookup/welfare/closure surface.
- **REF-0082-H → REF-0115-M** (`case-008 → case-013`): notification, officer reconciliation, or automatic merge changes what the citizen can cite when asking for the processing profile and changes the corresponding access surface.

Recurrence rules:
- The engine records only the chosen route needed for the current run. No browser persistence and no hidden dossier across restarts.
- Recurrence lives in case data and is materialized before display. The renderer does not contain narrative branch logic.
- Every recurrence variant adds one cold `Prior route update:` and overrides at least one later option field: label, outcome, or afterimage.
- A return is consequence, not reward. The humane route may preserve contact or standing; the middle route may widen procedure; the closing route may convert the earlier file into authoritative residue.
