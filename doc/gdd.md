...
- If the page still opens directly on the standby machine, the metadata files return plain handles, the correction surface can be reached without classifying the report too early, and release-relevant issues have a citeable response route instead of silent ingestion, stop.
