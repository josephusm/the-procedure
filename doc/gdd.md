...
- `review-window.txt`, review-window metadata, and JSON-LD subject links may name which filed corrections have standing before 1.0.0, how release-relevant pressure returns as a public response, closure reason, linked commit, release decision, or errata entry, which parent signal produced a changed review rule, and how the filer remains in the answerable chain instead of becoming intake data, provided the visible standby surface remains unchanged.
