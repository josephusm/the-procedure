...
- `review-window.txt`, review-window metadata, and JSON-LD subject links may name which filed corrections have standing before 1.0.0 and how release-relevant pressure returns as a public response, closure reason, linked commit, release decision, or errata entry, provided the visible standby surface remains unchanged.
...
- If the page still opens directly on the standby machine, the metadata files return plain handles, the correction surface can be reached without classifying the report too early, and release-relevant issues have a citeable response route instead of silent ingestion, and a contested misread response can still reopen public answer before release, stop.
